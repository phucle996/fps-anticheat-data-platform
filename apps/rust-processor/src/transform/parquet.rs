...
impl ParquetSerializer {
    ...
    pub fn record_batch_to_parquet_bytes(batch: &RecordBatch) -> Result<Vec<u8>> {
        ...
        let mut writer = ArrowWriter::try_new(&mut buffer, batch.schema(), Some(props))
            .map_err(|e| AppError::Parquet(format!("Khởi tạo Parquet ArrowWriter thất bại: {}", e)))?;

        // 3. Ghi RecordBatch
        writer
            .write(batch)
            .map_err(|e| AppError::Parquet(format!("Ghi RecordBatch sang Parquet thất bại: {}", e)))?;
    ...
    pub fn read_parquet_bytes(bytes: &[u8]) -> Result<Vec<RecordBatch>> {
        ...
        let builder = ParquetRecordBatchReaderBuilder::try_new(bytes_data)
            .map_err(|e| AppError::Parquet(format!("Tạo ParquetRecordBatchReaderBuilder thất bại: {}", e)))?;

        let reader = builder
            .build()
            .map_err(|e| AppError::Parquet(format!("Tạo Parquet RecordBatchReader thất bại: {}", e)))?;

        let mut batches = Vec::new();
        for batch_res in reader {
            let batch = batch_res.map_err(|e| AppError::Parquet(format!("Đọc RecordBatch từ Parquet byte stream thất bại: {}", e)))?;
            batches.push(batch);
        }
