...
fn create_sample_events() -> Vec<AnyEnvelope> {
    vec![
        ...
        AnyEnvelope::PlayerStat(EventEnvelope {
            schema_version: "1.0".to_string(),
            event_id: "b7793109ae2d60d5f3576cf8cf5ef0c23996b63e376ea8d2d67b7315e316ac02"
                .to_string(),
            op: "data.player_stat.match_summary".to_string(),
            event_time: None,
...
// Test_arrow_schema_transformation kiểm tra union schema không làm mất payload player-stat
#[test]
fn test_arrow_schema_transformation() {
    ...
    assert!(
        batch_res.is_ok(),
        "Chuyển đổi Arrow RecordBatch phải thành công"
    );
    ...
    assert_eq!(schema.field(21).name(), "kills");
    assert_eq!(schema.field(28).name(), "win_place_perc");
}
...
fn test_parquet_zstd_serialization_and_roundtrip() {
    ...
    assert!(
        parquet_bytes_res.is_ok(),
        "Mã hóa Parquet Zstd phải thành công"
    );

    let parquet_bytes = parquet_bytes_res.unwrap();
    assert!(
        !parquet_bytes.is_empty(),
        "Chuỗi byte Parquet không được rỗng"
    );
    ...
    assert!(
        read_batches_res.is_ok(),
        "Đọc lại Parquet round-trip phải thành công"
    );

    let read_batches = read_batches_res.unwrap();
    assert_eq!(read_batches.len(), 1);
    assert_eq!(read_batches[0].num_rows(), 2);
    assert_eq!(read_batches[0].num_columns(), 29);
